...
will be used to load and save RNAseq data, while the funcion [synTableQuery](https://www.rdocumentation.org/packages/synapseClient/versions/1.13-4/topics/synTableQuery) is used to gather the meta data. 
...
Meta data: `Y` is a single variable data frame consting of 301 observations.   
...
Call the `genes2hugo` function created before and use the labels from the RNSseq data to map genetype and geneid.
...
Use [read.delim](http://rfunction.com/archives/1441) to load the signiture from the saved 
file(pcbc-stemsig.tsv) and store it as a variable `w`. 
...
that mapped and values from the signiture gene set. 
...
create the `main` function which wrapes the all them. 
