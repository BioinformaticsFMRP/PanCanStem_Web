...
In the following workflow, we walk the reader through  training a stemness signature using normal pluripotent stem cell data from PCBC (training set) and applying it to score TCGA tumor samples (testing set). 
We provide the DNA methylation matrices for both datasets (They are subsets of the entire DNA methylation matrices, and contain 219 stem cell-probes signature). 
By executing the code below will reproduce mDNAsi scores, as presented in the manuscript. 
The code allows further exploration of custom probes list signature by replacing the DNA methylation matrices. You need to make sure both datasets (training and testing) contain the same probe list. Missing values (NA) are not allowed and we provide a function to overcome it. 
