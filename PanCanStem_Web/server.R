...
shinyServer(function(input, output, session) {
  output$tbl = DT::renderDataTable(
    datatable(
      ...
      formatStyle('cancer.type',
                  backgroundColor = styleEqual(sort(
                    unique(pd.all$cancer.type)
                  ), cancer.colors[sort(unique(pd.all$cancer.type))]))
  )
  
  updateSelectizeInput(session,
                       'cancertype',
                       choices = {
                         sort(unique(as.character(disease)))
                       },
                       server = TRUE)
  ...
  observeEvent(input$feature, {
    ...
    updateSelectizeInput(session, 'pathway', choices = {
      if (is.null(feature) || feature == "") {
        ...
        ret <-
          unique(levels(pd.maf.RNA[pd.maf.RNA$cancer.type %in% isolate({
            input$cancertype
          }) , get(feature)]),
          levels(pd.maf.450[pd.maf.450$cancer.type %in% isolate({
            input$cancertype
          }) , get(feature)]))
      } else {
        ret <- unique(as.character(pd.mRNA.prim[pd.mRNA.prim$cancer.type %in% isolate({
          input$cancertype
        }), get(feature)],
        pd.450.prim[pd.450.prim$cancer.type %in% isolate({
          input$cancertype
        }), get(feature)]))
      }
      ret
  ...
  volcano.values <- reactive({
    ...
    if (input$calculate) {
      ...
      primary <-
        as.character(pd.mRNA.prim[pd.mRNA.prim$sample.type %in% c("01", "03"), get("TCGAlong.id")])
      col <- "RNAss"
      ...
      test <-
        subset(pd,
               pd$cancer.type %in% isolate({
                 input$cancertype
               })  & pd$TCGAlong.id %in% primary)
      stats.rna <- test[order(test[, col, with = FALSE]), ] #rank samples
      stats.rna <-
        structure(stats.rna[, get(col)], names = as.character(stats.rna$TCGAlong.id))
      if (length(unique(test[, get(feature)])) < 2) {
        createAlert(
          ...
          style =  "danger",
          content = "There is not two levels for this combination. We cannot execute Gene Set Enrichment Analysis for this case.",
          append = FALSE
        )
        return(NULL)
      }
      pathways.rna <-
        as.list(unstack(test[, c("TCGAlong.id", feature), with = FALSE]))
      progress$set(value = 0.5, detail = "Executing Gene Set Enrichment Analysis")
      result.rna <-
        fgsea(
          pathways = pathways.rna,
          stats = stats.rna,
          nperm = nperm,
          minSize = 5,
          maxSize = 500
      ...
      primary <-
        as.character(pd.450.prim[pd.450.prim$sample.type %in% c("01", "03"), get("TCGAlong.id")])
      col <- "DNAss"
      test <-
        subset(pd,
               pd$cancer.type %in% isolate({
      ...
      stats.dna <-
        structure(stats.dna[, get(col)], names = as.character(stats.dna$TCGAlong.id))
      if (length(unique(test[, get(feature)])) < 2) {
        createAlert(
          session,
          "message",
  ...
  observeEvent(input$calculate , {
    output$plotEnrichmentDNA <- renderPlot({
      ...
      p2 <-
        plotEnrichment(ret$pathways.dna[["Mutant"]], ret$stats.dna)
      p <- ggarrange(p1,
                     p2,
                     labels = c("WT", "Mutant"),
                     ncol = 2)
  ...
  observeEvent(input$calculate , {
    output$plotEnrichmentRNA <- renderPlot({
      ...
      p2 <-
        plotEnrichment(ret$pathways.rna[["Mutant"]], ret$stats.rna)
      p <- ggarrange(p1,
                     p2,
                     labels = c("WT", "Mutant"),
                     ncol = 2)
  ...
  observeEvent(input$cancertype , {
    output$butterflyPlot <- renderPlotly({
      if (!is.null(input$cancertype) & input$cancertype != "") {
        p <- ggplotly(
          ggplot(
            pd.merg[pd.merg$pathway.RNA %in% "Mutant" &
                      pd.merg$cancer.type.DNA %in% input$cancertype, ],
            aes(
              x = NES.DNA,
              y = NES.RNA,
              tooltip = ID.RNA,
              color = c(padj.DNA < 0.05 |
                          padj.RNA < 0.05)
            )
          ) +
            geom_point() +
          ...
          layout(
            title = paste0(
              "DNAss vs RNAss Mutation Enrichment (",
              input$cancertype,
              ")"
            ),
  ...
  observeEvent(input$feature , {
    output$boxplot <- renderPlot({
      ...
      if (!is.null(input$cancertype) &
          input$cancertype != "" &
          !is.null(input$feature) & input$feature != "") {
        pd.dna <- as.data.frame(merge(pd.450.prim, pd.maf.450))
        plot <-
          pd.dna[pd.dna$cancer.type %in% input$cancertype, c("mDNAsi", input$feature)]
        colnames(plot) <- c("mDNAsi", "feature")
        plot$feature <-
          factor(plot$feature, levels = c("WT", "Mutant"))
        ...
        plot <-
          pd.dna[pd.dna$cancer.type %in% input$cancertype, c("mRNAsi", input$feature)]
        ...
        createAlert(
          ...
          title = "Error",
          style =  "danger",
          content = "Select cancer type and feature",
          append = FALSE
        )
        return(NULL)
      }
